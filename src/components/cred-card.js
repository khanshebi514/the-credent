...
export default function CredCard({image, title, point, background='white'}) {
  ...
  return (
    <section className={`flex justify-evenly items-center bg-${background} py-[50px] md:py-[100px] flex-col md:flex-row max-w-[1280px] m-auto`}>
    <div className="w-full md:w-[300px] flex justify-center items-center">
        <Image src={image} alt="back image" width={300} height={300}/>
    </div>
    <div className="space-y-3 w-full md:w-[500px] text-center md:text-start p-2">
        <h1 className="info-text font-bold my-3">{title}</h1>
